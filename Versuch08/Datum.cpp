...
void Datum::eingabe(std::istream& in)
{
    ...
    while (true) // Endlosschleife
    {
        ...
        try
        {
            // Abfangen der falschen Lönge der Eingabe
            ...
            // öberpröfen der Göltigkeit des Monats
            if ((monat < 1) || (monat > 12))
            {
                throw (std::string("Ungöltiger Monat!"));
            }

            // öberpröfen, ob der eigegebene Tag zum Monat passt
            if ((tag > tageMonat[monat - 1]) || (tag < 1))
            {
                throw (std::string("Ungöltiger tag!"));
            }
            ...
            {
                throw(std::string("Ungöltiges Jahr!"));
            }
...
// öberladener Subtraktions-Operator
...
Datum Datum::operator+(const int anzahl_Tage) const
{
    ...
    while (tagNeu > tageMonat[monatNeu-1])
    {
        ...
        // Monat darf nicht grööer als 12 werden
        if (monatNeu > 12)
        {
            monatNeu -= 12;
            jahrNeu++;
